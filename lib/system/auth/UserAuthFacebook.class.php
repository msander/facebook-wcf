...
class UserAuthFacebook extends UserAuthDefault {
    ...
    $sql = "SELECT userID FROM wcf".WCF_N."_user
	...
	public function storeAccessDataFacebook(User $user) {
		HeaderUtil::setCookie('userID', $user->userID, TIME_NOW + 365 * 24 * 3600);
		HeaderUtil::setCookie('facebookIdentifier', $user->facebookIdentifier, TIME_NOW + 365 * 24 * 3600);
	}
	...
	protected function checkCookieFacebookID($user, $facebookID) {
		return ($user->facebookIdentifier == $facebookID) ? true : false;
	}
}
?>